...
public class DialogueEditorWindow : EditorWindow
{
    ...
    private void ImportFromCSV()
    {
        ...
        string filePath = EditorUtility.OpenFilePanel("Select CSV Dialogue File", "", "csv");
    ...
    private void ImportAsNewDialogue(string filePath)
    {
        ...
        // Create new DialogueData instance
        DialogueData newDialogue = CreateInstance<DialogueData>();
        newDialogue.lines = lines.ToArray();
        newDialogue.choices = new DialogueData.DialogueChoice[0];
        newDialogue.nextDialogue = null;

        // Generate asset path
        string fileName = "ImportedDialogue";
        string assetPath = AssetDatabase.GenerateUniqueAssetPath($"{dialogueFolderPath}/{fileName}.asset");

        // Ensure folder exists
        EnsureFolderExists(dialogueFolderPath);
    ...
    private List<DialogueData.DialogueLine> ParseCSV(string filePath)
    {
        try
        {
            ...
            if (allLines.Length < 2)
            {
                EditorUtility.DisplayDialog("Invalid CSV",
                    "The CSV file must contain at least a header row and one data row.", "OK");
                return null;
            }
            ...
            if (speakerIndex == -1 || textIndex == -1)
            {
                EditorUtility.DisplayDialog("Invalid CSV Format",
                    "The CSV must have 'Speaker' and 'Text' columns in the header row.", "OK");
                return null;
            }

            // Parse dialogue lines
            List<DialogueData.DialogueLine> dialogueLines = new List<DialogueData.DialogueLine>();
            int lineNumber = 2; // Start at 2 (1-indexed, skip header)
            ...
            for (int i = 1; i < allLines.Length; i++)
            {
                ...
                // Split by comma (simple split - doesn't handle quoted commas)
                string[] columns = line.Split(',');

                if (columns.Length <= Mathf.Max(speakerIndex, textIndex))
                {
                    Debug.LogWarning($"[Dialogue Editor] Line {lineNumber}: Not enough columns, skipping.");
                    lineNumber++;
                    continue;
                }

                // Parse speaker
                string speakerString = columns[speakerIndex].Trim();
                Speaker speaker;

                if (!System.Enum.TryParse(speakerString, true, out speaker))
                {
                    Debug.LogError($"[Dialogue Editor] Line {lineNumber}: Invalid speaker '{speakerString}'. " +
                        "Valid values are: MC, Queen, Narrator. Skipping line.");
                    lineNumber++;
                    continue;
                }

                // Parse text
                string text = columns[textIndex].Trim();

                // Create DialogueLine
                DialogueData.DialogueLine dialogueLine = new DialogueData.DialogueLine
                {
                    speaker = speaker,
                    text = text,
                    voiceClip = null,
            ...
            {
                EditorUtility.DisplayDialog("No Valid Lines",
                    "The CSV file contains no valid dialogue lines.", "OK");
                return null;
            }

            Debug.Log($"[Dialogue Editor] Successfully parsed {dialogueLines.Count} dialogue lines from CSV.");
        ...
        catch (System.Exception ex)
        {
            ...
            Debug.LogError($"[Dialogue Editor] CSV Import Error: {ex}");
            return null;
        }
    }
